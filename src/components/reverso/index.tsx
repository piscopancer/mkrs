...
export default async function Reverso({ ch, links = false, ...htmlProps }: ComponentProps<'article'> & { ch: string; links?: boolean }) {
    ...
    <>
      ...
      <article {...htmlProps} className={clsx(htmlProps.className, 'rounded-lg border-2 border-zinc-800')}>
        <header className='flex items-center bg-zinc-800/50 text-sm'>
          <Tooltip content={`type: ${search.type}, links: ${links}`} delay={2000}>
        ...
        <div className='px-3 py-2'>
          {search.type === 'one' && (
            <ul className={clsx('text-zinc-400', links ? '-mx-2 flex flex-wrap' : '')}>
              {search.translations.map((tr, i, arr) => (
                <Fragment key={i}>
                  <li className={clsx(links ? 'block' : 'inline')}>
                    {links ? (
                  ...
                  {!links && <span className='cursor-default'>{i < arr.length - 1 && ', '}</span>}
                </Fragment>
              ))}
            </ul>
          )}
