    ...
    <div {...attr} className={clsx(attr.className, 'relative')}>
      <ul className='flex justify-between rounded-full bg-zinc-900 p-1'>
        {props.options.map((o, i) => (
          <li key={i} className={clsx('size-4 rounded-full duration-100', props.selected(o) ? 'scale-110 bg-zinc-300' : 'bg-zinc-600')}></li>
        ))}
      </ul>
      <menu className='absolute -inset-8 flex justify-stretch'>
        {props.options.map((o, i) => (
