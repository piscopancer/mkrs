  ...
  return (
    <section {...compProps} className={clsx(compProps.className)}>
      ...
      <ul className={clsx('grid grid-cols-[1fr_1fr] gap-2 gap-x-8 gap-y-2 max-md:flex max-md:flex-col max-md:gap-x-4', collapsed && 'hidden max-md:hidden')} data-search>
        {ch.map((ch, i) => (
          <li key={i} className='contents rounded-lg py-2 text-zinc-400 max-md:block max-md:py-1'>
            <Link prefetch={false} href={`/search/${ch.heading}`} className='w-fit self-start text-lg max-md:mb-1' data-custom>
              {ch.heading}
            </Link>
            <div className='max-md:text-base'>{ch.innerHtml && stringToReact(ch.innerHtml)}</div>
          </li>
        ))}
      </ul>
    </section>
