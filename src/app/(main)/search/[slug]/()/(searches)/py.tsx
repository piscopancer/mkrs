        ...
        <ul className='grid grid-cols-[min-content,min-content,1fr] gap-x-6 max-md:flex max-md:flex-col'>
          {props.response.words.map((word, i) => (
            <li key={i} className='contents'>
              <Link prefetch={false} href={`/search/${word.ch}`} className='col-span-3 grid grid-cols-[subgrid] items-center border-b-2 border-dashed border-zinc-800 py-2 text-zinc-400 hover:border-zinc-700 hover:text-zinc-200 max-md:grid-cols-[min-content,1fr]'>
