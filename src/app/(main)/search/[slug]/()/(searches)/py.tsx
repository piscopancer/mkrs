        ...
        <ul className='grid grid-cols-[min-content,min-content,1fr] gap-x-6 max-md:flex max-md:flex-col'>
          {props.response.words.map((word, i, arr) => (
            <li key={i} className='contents'>
              <Link href={`/search/${word.ch}`} className={clsx('col-span-3 grid grid-cols-[subgrid] items-center py-2 text-zinc-400 duration-100 hover:text-zinc-200 max-md:grid-cols-[min-content,1fr]', i !== arr.length - 1 && 'border-b border-zinc-800')}>
