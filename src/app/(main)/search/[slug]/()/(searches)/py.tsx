        ...
        <ul className='grid grid-cols-[min-content,min-content,1fr] gap-x-6 max-md:flex max-md:flex-col'>
          {props.response.words.map((word, i, arr) => (
            <li key={i} className='group contents'>
              <Link href={`/search/${word.ch}`} className='col-span-3 -mx-2 grid grid-cols-[subgrid] items-center rounded-md p-2 hover:bg-zinc-800/50 group-last:border-0 max-md:grid-cols-[min-content,1fr]'>
