...
export default function RootLayout({ children }: { children: React.ReactNode }) {
  return (
    <html lang='ru'>
      <body className={classes(fonts.sans, 'text-stone-200 bg-stone-900')}>
        <div className='grid grid-cols-[min-content,1fr] grid-rows-[min-content,1fr] h-screen'>
          <div className='grid [grid-template-areas:"stack"] w-20 h-20'>
            ...
            <div className='h-1/3 w-[2px] bg-stone-700 [grid-area:stack] justify-self-end self-center' />
          </div>
          <h1 className={classes(fonts.display, 'self-center ml-4')}>
            МКРС <span className='text-xs text-stone-600 ml-4'>// БКРС ПРОКСИ</span>
          </h1>
          <aside className='px-3 flex flex-col justify-center'>
            <ul className='flex flex-col gap-2'>
              <Tooltip content='Сохраненные' side='right' sideOffset={6}>
                <button className='hover:bg-stone-800 rounded-full py-2 flex justify-center'>
              ...
              <hr className='border-stone-700 border my-2 w-8 mx-auto' />
          ...
          <section className='border-2 border-stone-800 mr-3 mb-3 rounded-lg'>{children}</section>
        </div>
      </body>
    </html>
  )
